...
class plugintype_analyticsindicator extends plugintype_base {
...
function coursereport_engagement_sort_indicators($a, $b) {
    global $SESSION;
...
function coursereport_engagement_sort_risks($a, $b) {
    global $SESSION;
...
function coursereport_engagement_update_indicator($courseid, $new_weights, $configdata = array()) {
    global $DB;

    $weights = array();
    if ($weightrecords = $DB->get_records('coursereport_engagement', array('course' => $courseid))) {
    ...
    foreach ($new_weights as $indicator => $weight) {
        ...
        if (!isset($weights[$indicator])) {
            ...
            $DB->update_record('coursereport_engagement', $weights[$indicator]);
        }
    }
}
