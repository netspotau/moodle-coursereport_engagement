...
$PAGE->set_url('/course/report/engagement/index.php', $pageparams);
$PAGE->set_pagelayout('report');

$course = $DB->get_record('course', array('id' => $id), '*', MUST_EXIST);
if ($userid) {
    $user = $DB->get_record('user', array('id' => $userid), 'id, firstname, lastname, email', MUST_EXIST);
    $PAGE->navbar->add(fullname($user), new moodle_url('/course/report/engagement/index.php', $pageparams));
}
...
if (!$userid) {
    add_to_log($course->id, "course", "report engagement", "course/report/engagement/index.php?id=$course->id", $course->id);
} else {
    add_to_log($course->id, "course", "report engagement",
        "course/report/engagement/index.php?id=$course->id&userid=$user->id", $course->id);
}

$stradministration = get_string('administration');
$strreports = get_string('reports');
$renderer = $PAGE->get_renderer('coursereport_engagement');
...
echo $OUTPUT->heading(get_string($heading, 'coursereport_engagement', $info));
...
$weightings = $DB->get_records_menu('coursereport_engagement', array('course' => $id), '', 'indicator, weight');
...
if (!$userid) { // Course report.
    ...
    if ($tsort && isset($SESSION->flextable['engagement-course-report'])) {
        ...
        if ($tsort == 'total') {
            uasort($data, 'coursereport_engagement_sort_risks');
        } else if (preg_match('/^indicator_(.*)/', $tsort, $matches)) {
            uasort($data, 'coursereport_engagement_sort_indicators');
        }
    } else {
        uasort($data, 'coursereport_engagement_sort_risks');
    }
