...
$strpluginname = get_string('pluginname', 'coursereport_engagement');
...
$PAGE->navbar->add(get_string('updatesettings', 'coursereport_engagement'), $url);
$PAGE->set_title("$course->shortname: $strpluginname");
$PAGE->set_heading($course->fullname);

$indicators = get_plugin_list('engagementindicator');
$mform = new coursereport_engagement_edit_form(null, array('id' => $id, 'indicators' => $indicators));
...
if ($mform->is_cancelled()) {
    ...
    coursereport_engagement_update_indicator($id, $weights, $configdata);
}

// Get current values and populate form.
$data = array();
if ($indicators = $DB->get_records('coursereport_engagement', array('course' => $id))) {
